...
export default {
  ...
  methods: {
    ...
    async deletarEndereco(id) {
      swal('Deseja deletar o cep selecionado?').then((value) => {
        if (value) {
  ...
  <div class="container">
    <div class="mb-3 d-flex align-items-center justify-content-between">
      <input
        ...
        id="filtro"
        placeholder="Digite o cep ou logradouro"
        v-model="filtro"
      />
      <button type="button" class="btn btn-outline-success mb-3 ml-2" @click="buscarEndereco()">
        Buscar
      </button>
    </div>
